...
const FIELDS: Record<string, { label: string; min: number; max: number; step: number; placeholder: string }> = {
  ...
  promotions: { label: '晋升次数', min: 0, max: 20, step: 1, placeholder: '如 2' },
  commute_minutes: { label: '通勤时长(分钟)', min: 0, max: 300, step: 5, placeholder: '如 45' },
}

export default function AttritionRiskPage() {  const [info, setInfo] = useState<any>(null)
  ...
  const run = async () => {
    ...
    setLoading(true)
    try {
      const r: any = await http.post('/api/tools/attrition-risk', input)
  ...
  return (
    <div>
      ...
      <Tag color="cyan" style={{ borderRadius: 100 }}>ML 教学示例</Tag>
      <Text type="secondary" style={{ marginLeft: 8 }}>SVM 支持向量机 · 二分类 · 1500 条合成数据</Text>

      {info?.error && <Alert type="error" style={{ marginTop: 16 }} showIcon message={info.error} />}

      {info && !info.error && (
        <Row gutter={16} style={{ marginTop: 16 }}>
          <Col xs={12} md={4}><Card size="small" style={{ borderRadius: 12 }}><Statistic title="样本数" value={info.data_points} /></Card></Col>
          <Col xs={12} md={4}><Card size="small" style={{ borderRadius: 12 }}><Statistic title="准确率" value={`${(m.accuracy * 100).toFixed(1)}%`} /></Card></Col>
          ...
          <Col xs={12} md={4}><Card size="small" style={{ borderRadius: 12 }}><Statistic title="F1" value={`${(m.f1 * 100).toFixed(1)}%`} /></Card></Col>
        </Row>
      )}
